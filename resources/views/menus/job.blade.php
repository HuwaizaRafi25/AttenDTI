                            ...
                            <div>
                                ...
                                <div id="jobTypeContainer" class="flex flex-wrap gap-2">
                                    @foreach ($jobTypes as $jobType)
                                        <div class="w-max p-2 cursor-pointer border border-gray-300 rounded-xl job-type-item" data-id="{{ $jobType->id }}">
                                            {{ $jobType->job_type_name }}
                                        </div>
                                    @endforeach
                                    <div id="addJobTypeBtn" class="w-max p-2 cursor-pointer border border-blue-500 text-blue-500 rounded-xl hover:bg-blue-50 transition-colors duration-150">
                                        + Job Type
                                    </div>
                                </div>
                            </div>
            ...
            <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <a href="/job_detail">
                    <div class="border rounded-lg p-4 hover:shadow-lg transition-shadow">
                        ...
                        <p class="mt-3 text-sm text-gray-600 line-clamp-2">Deskripsi pekerjaan...</p>
...
<script>
    $(document).ready(function() {
        // Form untuk menambahkan job type
        $('#addJobTypeForm').on('submit', function(e) {
            e.preventDefault();

            var newJobType = $('#newJobType').val();
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Tutup modal add job type
                    $('#addJobTypeModal').addClass('hidden').removeClass('flex');

                    // Tambahkan elemen baru ke dalam jobTypeContainer
                ...
                error: function(xhr) {
                    ...
                    if (errors && errors.job_type_name) {
                        alert(errors.job_type_name[0]);
                    } else {
                        alert('Terjadi kesalahan saat menambahkan job type.');
                    }
                }
            });
        });

        // Event klik untuk tombol Cancel pada modal add job type
        $('#cancelAddJobType').on('click', function() {
            $('#addJobTypeModal').addClass('hidden').removeClass('flex');
            $('#newJobType').val('');
        });

        // Buka modal add job type saat tombol + Job Type diklik
        $('#addJobTypeBtn').on('click', function() {
            $('#addJobTypeModal').removeClass('hidden').addClass('flex');
        });

        // Event untuk toggle selected job type
        $('#jobTypeContainer').on('click', '.job-type-item', function() {
            $(this).toggleClass('border-blue-500 bg-blue-50');
        });
        ...
        // Event untuk input style pada focus/blur
        const formInputs = document.querySelectorAll('input, textarea');
        formInputs.forEach(input => {
            input.addEventListener('focus', () => {
                input.classList.add('border-blue-500', 'ring', 'ring-blue-200', 'ring-opacity-50');
            });
            input.addEventListener('blur', () => {
                input.classList.remove('border-blue-500', 'ring', 'ring-blue-200', 'ring-opacity-50');
            });
        });
    });

    // Fungsi untuk toggle dropdown (Job Type pada sidebar)
    function toggleDropdown() {
        const dropdown = document.getElementById('jobTypeDropdown');
        dropdown.classList.toggle('hidden');
    }

    // Tambahan: fungsi untuk menambah input qualification/responsibility
    ...
    addQualificationBtn.addEventListener('click', () => addInput(qualificationsContainer, 'Enter qualification'));
    addResponsibilityBtn.addEventListener('click', () => addInput(responsibilitiesContainer, 'Enter responsibility'));

    // Menambahkan input default untuk qualification dan responsibility
